-- The legacy unused_cp_refund_* columns are retained for compatibility, but now represent
-- a fixed reward: 500 coins when at least 10% of the granted CP budget remains unspent.
-- The API calculates eligibility from the final commissioner-approved build. This function
-- remains the atomic, one-time wallet credit boundary.
