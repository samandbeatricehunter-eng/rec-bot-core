-- Server-side aggregation for User Ratings (apps/api ratings.service.ts computeUserRatings).
-- Replaces a select("*") over every rec_team_game_stats row for a league/season (hundreds of
-- wide rows, ~1125 bytes each) with one aggregated row per user computed inside Postgres.
--
-- Faithfully replicates the quarantine + totals logic from
-- apps/api/src/modules/box-score-intelligence/game-profile.ts (rowToGameStats) and
-- apps/api/src/modules/box-score-intelligence/aggregate.ts (seasonTotalsFromGames) for the
-- specific fields computeUserRatings actually reads: passing/rushing yards, first downs,
-- turnovers committed/forced, season red-zone-offense %, and games played — a game is
-- excluded entirely from a user's totals if it fails any of the same sanity checks the JS
-- path applies (turnovers/interceptions/fumbles out of [0,10], yards-per-play/rush/pass out
-- of [0,25], third/fourth-down or pass-completion "made" exceeding "attempted").
