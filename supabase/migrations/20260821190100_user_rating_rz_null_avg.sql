-- User rating was coalescing missing red-zone % to 0, which treated unmapped
-- companion rows as 0% RZ and crushed the production score. Average only the
-- games that actually stored a red-zone percentage.
...
as $$
  with rows as (
    select
      ...
      t.red_zone_off_percentage as red_zone_off_percentage,
