-- Coach Rating is removed app-wide (badges removal pass) and User Rating no longer
-- factors badges — it now blends win/performance signals, per-game production, and
-- consistency of that production (stddev of total yards/game). Extend the RPC to
-- return wins/losses/ties and a population stddev of total yards per game alongside
-- the existing per-game sums, so ratings.service.ts can compute all three components
-- from one round trip instead of a second query. Same career-scope semantics as
-- before (p_season_number <= 0 aggregates every season).
...
as $$
  ...
  select
    ...
    count(*) filter (where f.result = 'tie') as ties,
    ...
    coalesce(stddev_pop(f.off_pass_yards + f.off_rush_yards), 0) as total_yards_stddev
  from flagged f
