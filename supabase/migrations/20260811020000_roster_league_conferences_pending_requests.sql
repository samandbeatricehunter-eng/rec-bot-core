-- rec_roster_league_conferences (powers /openteams and the web hub's Open Teams view) only
-- ever reflected rec_team_assignments, so a team with a pending (unapproved) team-link request
-- still showed up as available to request — letting a second member submit a competing request
-- for the same team. Adds hasPendingRequest to each team payload so callers can treat it the
-- same as an already-linked team.
