...
begin
  ...
  with players as (
    select
      ...
      coalesce(
        rp.cap_hit,
        case when rs.raw_payload->>'capHit' ~ '^-?[0-9]+$' then (rs.raw_payload->>'capHit')::bigint end
      ) as cap_hit,
      coalesce(
        rp.contract_years_left,
        rs.contract_years_left,
        case when rs.raw_payload->>'contractYearsLeft' ~ '^-?[0-9]+$' then (rs.raw_payload->>'contractYearsLeft')::integer end
      ) as contract_years_left,
      case
        ...
        when upper(coalesce(rs.position, '')) in ('WILL', 'MIKE', 'SAM', 'MLB', 'LOLB', 'ROLB', 'LB') then 7
      ...
      case
        ...
        when upper(coalesce(rs.position, '')) in ('WILL', 'MIKE', 'SAM', 'MLB', 'LOLB', 'ROLB', 'LB') then 'Linebackers'
      ...
      case
        ...
        when upper(coalesce(rs.position, '')) in ('LEDGE', 'REDGE', 'LE', 'RE', 'DT', 'DL', 'EDGE', 'WILL', 'MIKE', 'SAM', 'MLB', 'LOLB', 'ROLB', 'LB', 'CB', 'FS', 'SS', 'DB', 'S') then 'defense'
        when upper(coalesce(rs.position, '')) in ('K', 'P', 'LS') then 'special'
        else 'other'
      end as group_side,
      case
        when upper(coalesce(rs.position, '')) = 'LT' then 1
        when upper(coalesce(rs.position, '')) = 'LG' then 2
        when upper(coalesce(rs.position, '')) = 'C' then 3
        when upper(coalesce(rs.position, '')) = 'RG' then 4
        when upper(coalesce(rs.position, '')) = 'RT' then 5
        when upper(coalesce(rs.position, '')) = 'LEDGE' then 1
        when upper(coalesce(rs.position, '')) = 'DT' then 2
        when upper(coalesce(rs.position, '')) = 'REDGE' then 3
        when upper(coalesce(rs.position, '')) = 'WILL' then 1
        when upper(coalesce(rs.position, '')) = 'MIKE' then 2
        when upper(coalesce(rs.position, '')) = 'SAM' then 3
        when upper(coalesce(rs.position, '')) = 'CB' then 1
        when upper(coalesce(rs.position, '')) = 'FS' then 2
        when upper(coalesce(rs.position, '')) = 'SS' then 3
        else 50
      end as position_order
    from public.rec_roster_snapshots rs
    left join public.rec_players rp
      on rp.id = rs.player_id
    where rs.league_id = v_league_id
  ...
  groups as (
    select
      ...
      jsonb_agg(
        ...
        order by position_order, ovr desc, name
      ) as members,
      count(*) as player_count
    from players
    group by group_order, group_label, group_side
  )
  select jsonb_build_object(
    ...
    'groups', coalesce(
      jsonb_agg(
        jsonb_build_object('label', group_label, 'side', group_side, 'members', members)
