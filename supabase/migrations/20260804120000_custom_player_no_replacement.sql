-- Backlog #42 — non-seeded-league custom-player purchases.
-- In a CFB league that was never seeded from the baseline roster, a team's roster is empty,
-- so there is no active player to "replace". Allow replacement_player_id to be null: the
-- build then creates a brand-new player with no outgoing row to delete. The API still
-- requires an active replacement whenever the team has players; null replacement is only
-- permitted for empty (unseeded) rosters.
...
begin
  ...
  insert into public.rec_players (
    ...
    case when v_build.game_family = 'CFB' then nullif(v_identity->>'hometownCity','') else null end,
    case when v_build.game_family = 'CFB' then nullif(v_identity->>'hometownState','') else null end,
