-- Phase 1 cleanup (Comp removal): public.refresh_power_rankings() (created in
-- 20260805111500_power_rankings_pg_cron.sql) computed both a 'dynasty' scope and a 'comp' scope
-- per game. The Comp product is being removed, so this re-creates the function with the comp
-- scope block dropped entirely -- dynasty scoring is untouched.
