...
## Post-Advance Media Day Experience

Every advance creates a user-specific, server-enforced Advance Experience.

Sequence:
Media Day → previous-stage Rewards Recap → Media Posture recap → Weekly Challenge Reveal → Dev Opportunity reveal if applicable → Enter League.

Media Day is gate-only, not normal navigation. Completed answers are immutable and may only be viewed later through a read-only archive.

Non-RTI uses one continuous team press conference with 2–4 questions normally and up to 6 only on unusually eventful advances.

RTI interviews are subject-specific and direct-address the stored Owner, Offensive Player and/or Defensive Player by name. Ask only relevant subjects; normally 1–3 questions, maximum 4 per subject.

The full UI/orchestration contract is in:
- `05_UI/MEDIA_DAY_GATE_ORCHESTRATION.md`
- `05_UI/POST_ADVANCE_EXPERIENCE.md`

Social/media personality and discussion behavior is defined in:
- `02_TWEETS_MEDIA/media_personality_profiles.json`
- `02_TWEETS_MEDIA/SOCIAL_SYSTEM_ARCHITECTURE.md`
- `02_TWEETS_MEDIA/social_event_playbooks.json`
- `02_TWEETS_MEDIA/social_relationship_matrix.json`
