...
## Media / Advance Experience implementation note

Implement Media Day as a league access gate, not a dismissible client-only modal.

Required server concepts:
- advance experience snapshot
- unresolved gate state
- immutable answer records
- reporter assignment
- subject phase
- reward recap snapshot
- media posture snapshot
- challenge reveal state
- Dev opportunity reveal state
- resume checkpoint

Run current-repo discovery before naming tables/routes so this architecture is merged into existing Media Day, challenge, economy and progression services rather than duplicated.
