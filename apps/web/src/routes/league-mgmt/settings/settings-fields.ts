...
export const SETTINGS_CATEGORIES: SettingsCategory[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "devUpgradeCapMode", label: "Dev Upgrade Cap Type", type: "enum", options: [{ value: "total_purchases", label: "Limit total upgrade purchases per team" }, { value: "players_per_season", label: "Limit number of players who can be upgraded" }], dependsOn: (d) => Boolean(d.coinEconomyEnabled && d.devUpgradesEnabled) },
      { key: "devUpgradesSeasonCap", label: "Dev Upgrades Season Cap (purchase actions)", type: "number", min: 0, max: 20, dependsOn: (d) => Boolean(d.coinEconomyEnabled && d.devUpgradesEnabled && (d.devUpgradeCapMode ?? "total_purchases") === "total_purchases"), resetTo: 0 },
      { key: "devUpgradesPlayerCap", label: "Dev Upgrades Player Cap (distinct players per team)", type: "number", min: 0, max: 20, dependsOn: (d) => Boolean(d.coinEconomyEnabled && d.devUpgradesEnabled && d.devUpgradeCapMode === "players_per_season"), resetTo: 0 },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "cpuTradingRestriction", label: "CPU Trading Restriction Details", type: "textarea", gameFilter: notCfb, dependsOn: (d) => d.cpuTradingPolicy === "restricted" },
