...
export function CustomPlayerWizard({ guildId, onPurchased }: { guildId: string; onPurchased: () => void }) {
  ...
  return <div className="custom-player-wizard"><p className="hub-eyebrow">Step {step} of 5</p>
    ...
    {step === 5 && <><h4>Review</h4><p><strong>{identity.firstName} {identity.lastName}</strong> · {position} · {effectiveArchetypeKey.replaceAll("_"," ")} (auto-detected)</p><p>{pkg.displayName} · <CoinAmount amount={pkg.coinPrice}/> · {evaluation?.displayOverall} OVR ({evaluation?.rawOverall} raw)</p><p className="form-hint"><strong>88 OVR maximum:</strong> if the applied player evaluates above 88 OVR, the commissioner must reduce ratings to reach the cap. Identity or rating edits are logged and sent to you; position, archetype, and development trait remain locked to this purchase.</p><p>{evaluation?.totalCost}/{evaluation?.creationPoints} CP · {evaluation?.pointsRemaining} unspent · <strong>{Math.ceil((evaluation?.pointsRemaining ?? 0) * .10)} coin refund after application</strong></p>{positionLocked ? <p className="form-hint">Replacing: <strong>{(() => { const p = config.replacementPlayers.find((player: any) => player.id === replacementPlayerId); return p ? (p.full_name ?? `${p.first_name} ${p.last_name}`) : "selected player"; })()}</strong> ({position}) — chosen in Step 3, locked to this purchase.</p> : config.replacementRequired ? <label>Replace active player{game === "CFB" ? ` (must be ${position} — CFB recruits inherit that player's position)` : " (any position)"}<select className="form-input" value={replacementPlayerId} onChange={(e) => {
