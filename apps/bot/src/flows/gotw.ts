...
export async function handleGotwConfirm(interaction: ButtonInteraction, buildAdvanceRows: () => ActionRowBuilder<ButtonBuilder>[]) {
  ...
  return interaction.editReply({ embeds: [new EmbedBuilder().setTitle("GOTW Voting Open").setDescription(`Opened web voting for ${stageLabel(stage, currentWeek, leagueGame)}. Users can vote from the REC site.`)], components: buildAdvanceRows() });
}
