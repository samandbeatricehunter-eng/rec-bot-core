...
export async function handlePowerRankingsSlash(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const teams = (result.teams ?? []) as Array<{ teamName: string; rank: number; prevRank: number | null; change: number | null }>;
    if (!teams.length) {
      await interaction.editReply({ content: "No power rankings available yet." });
      return;
    }
    const lines = teams.map((team) => `${team.rank}. **${team.teamName}**${changeArrow(team.change)}`);
