...
// Post-setup schedule entry only covers the regular season; the postseason is
// generated by the advance flow, not manually entered.
function postSetupMaxWeek(session: { game: string | null }) {
  return regularSeasonWeeks(session.game);
}
...
const postSetupSessions = new Map<string, { guildId: string; franchiseYearOne: boolean; game: string | null }>();

export function markPostSetupActive(userId: string, guildId: string, franchiseYearOne: boolean, game: string | null = null) {
  postSetupSessions.set(userId, { guildId, franchiseYearOne, game });
}
...
export async function renderScheduleMenu(interaction: ButtonInteraction) {
  ...
  if (!interaction.guildId) {
    return interaction.editReply({
      embeds: [new EmbedBuilder().setTitle("Schedule").setDescription("Open this league on the REC site (/viewleague) to view schedules.")],
    ...
    return interaction.editReply({
      embeds: [
        buildScheduleEmbed({
          ...
          game: schedule?.league?.game ?? null
        })
      ],
      components: buildScheduleRows()
    });
...
export async function handleScheduleSos(interaction: ButtonInteraction) {
  ...
  if (!interaction.guildId) {
    return interaction.editReply({
      embeds: [new EmbedBuilder().setTitle("Strength of Schedule").setDescription("Open this league on the REC site (/viewleague) to view SOS.")],
      components: buildScheduleRows(),
    });
  }
  try {
...
export async function handleSchedulePowerRankings(interaction: ButtonInteraction) {
  ...
  if (!interaction.guildId) {
    return interaction.editReply({
      embeds: [new EmbedBuilder().setTitle("Power Rankings").setDescription("Open this league on the REC site (/viewleague) to view power rankings.")],
      components: buildScheduleRows(),
    });
  }
  try {
...
export async function startScheduleTeamSelect(interaction: ButtonInteraction) {
  await interaction.deferUpdate();
  if (!interaction.guildId) {
    return interaction.editReply({ embeds: [new EmbedBuilder().setTitle("View Other Teams").setDescription("Open this league on the REC site (/viewleague or /openteams).")], components: buildScheduleRows() });
  }
...
export async function handleScheduleStats(interaction: ButtonInteraction) {
  await interaction.deferUpdate();
  if (!interaction.guildId) return interaction.editReply({ embeds: [new EmbedBuilder().setTitle("Stats").setDescription("Open this league on the REC site (/viewleague or /openteams).")], components: buildScheduleRows() });
...
export async function handleManualScheduleNextWeek(interaction: ButtonInteraction, force = false) {
  ...
  const maxWeek = session.mode === "post_setup" ? postSetupMaxWeek(session) : maxSeasonWeek(session.game);
  if (session.weekNumber >= maxWeek) {
    session.notice = session.mode === "post_setup"
      ? `Week ${maxWeek} is the last regular-season week for setup. Finish entering matchups, then press **Finish Setup**.`
...
export async function handleManualScheduleBack(interaction: ButtonInteraction) {
  ...
  if (session?.mode === "post_setup") {
    manualScheduleSessions.delete(sessionKey(session.guildId, session.userId));
    return interaction.update(renderPostSetupScheduleInputChoice(session.game ?? null));
  }
...
export async function startPostSetupScheduleStep(interaction: ButtonInteraction) {
  ...
  return interaction.editReply(renderPostSetupScheduleInputChoice(postSetup?.game ?? null));
}

async function loadPostSetupScheduleReview(interaction: ButtonInteraction) {
  const postSetup = postSetupSessions.get(interaction.user.id);
  await interaction.editReply({
    embeds: [new EmbedBuilder().setTitle("Loading Schedule...").setDescription(`Fetching Weeks 1–${regularSeasonWeeks(postSetup?.game ?? null)} for review.`)],
    components: [],
  });
  try {
    const result = await recApi.listScheduleSeason({ guildId: interaction.guildId! });
    const game = result?.league?.game ?? postSetup?.game ?? null;
    const session: ScheduleViewSession = {
      guildId: interaction.guildId!,
      userId: interaction.user.id,
      game,
      ...
      seasonNumber: Number(result?.league?.seasonNumber ?? 1),
      pageIndex: 0,
      weeks: filterPostSetupWeeks(result?.weeks ?? [], game),
      mode: "post_setup",
    };
    scheduleViewSessions.set(sessionKey(interaction.guildId!, interaction.user.id), session);
  ...
  return [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      new ButtonBuilder()
        ...
        .setDisabled(session.mode === "post_setup" && session.weekNumber >= postSetupMaxWeek(session)),
    ),
    new ActionRowBuilder<ButtonBuilder>().addComponents(
...
function formatViewWeekLabel(weekNumber: number, game: string | null) {
  if (isRegularSeasonWeek(weekNumber, game)) return `Week ${weekNumber}`;
  const label = stageLabel(stageForWeek(weekNumber, game), weekNumber, game);
  return label.endsWith("Week") ? label : `${label} Week`;
}
...
function buildPostSetupWeekEmbed(session: ScheduleViewSession) {
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: `Page ${session.pageIndex + 1} of ${postSetupMaxWeek({ game: session.game ?? null })}` });
}
...
function buildScheduleWeekEmbed(session: ScheduleViewSession) {
  ...
  return new EmbedBuilder()
    .setTitle(`${session.leagueName ?? "League"} Schedule - ${formatViewWeekLabel(page.weekNumber, session.game ?? null)}`)
    .setDescription(lines.slice(0, 4096))
    .setFooter({ text: `Season ${session.seasonNumber} - Page ${session.pageIndex + 1}/${session.weeks.length || maxSeasonWeek(session.game ?? null)}` });
}
