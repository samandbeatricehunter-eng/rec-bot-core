...
export const recUserLeagueHistory = pgTable("rec_user_league_history", {
  id: uuid("id").primaryKey(),
  userId: uuid("user_id").notNull().references(() => recUsers.id),
  leagueId: uuid("league_id").notNull(),
  leagueName: text("league_name").notNull(),
  game: text("game").notNull(),
  firstJoinedAt: timestamp("first_joined_at", { withTimezone: true, mode: "string" }),
  ...
  seasonsParticipated: integer("seasons_participated").notNull().default(0),
  firstSeason: integer("first_season"),
  lastSeason: integer("last_season"),
  record: jsonb("record").$type<Record<string, unknown>>().notNull(),
  statistics: jsonb("statistics").$type<Record<string, unknown>>().notNull(),
  achievements: jsonb("achievements").$type<Record<string, unknown>>().notNull(),
  economy: jsonb("economy").$type<Record<string, unknown>>().notNull(),
  wagerHouse: jsonb("wager_house").$type<Record<string, unknown>>().notNull(),
  wagerPeer: jsonb("wager_peer").$type<Record<string, unknown>>().notNull(),
  snapshotVersion: integer("snapshot_version").notNull().default(1),
  createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull(),
});

export const recUserLeagueTeamTenures = pgTable("rec_user_league_team_tenures", {
  id: uuid("id").primaryKey(),
  historyId: uuid("history_id").notNull().references(() => recUserLeagueHistory.id),
  userId: uuid("user_id").notNull().references(() => recUsers.id),
  leagueId: uuid("league_id").notNull(),
...
export const recAdminIncidents = pgTable("rec_admin_incidents", {
  id: uuid("id").primaryKey(),
...
export const recLeagueConfiguration = pgTable("rec_league_configuration", {
  ...
  sliderPresetId: text("slider_preset_id"),
  sliderCatalogVersion: text("slider_catalog_version"),
  sliderSettings: jsonb("slider_settings").$type<Record<string, number>>().notNull().default({}),
...
export const recHighlightPosts = pgTable("rec_highlight_posts", {
  ...
  discordChannelId: text("discord_channel_id"),
  discordMessageId: text("discord_message_id"),
  discordAttachmentIndex: integer("discord_attachment_index").notNull().default(0),
