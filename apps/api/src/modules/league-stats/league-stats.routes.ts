...
import { getLeagueStats } from "./league-stats.service.js";

export async function leagueStatsRoutes(app: FastifyInstance) {
  app.post("/v1/hub/league-stats", async (request, reply) => {
    try {
      const body = z.object({ guildId: z.string().min(1), teamId: z.string().uuid().nullable().optional(), position: z.string().max(20).nullable().optional() }).parse(request.body);
