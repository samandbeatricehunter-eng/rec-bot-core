...
export async function wagerRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/wagers/settle", async (request, reply) => {
    try {
      ...
      const body = z.object({ guildId: z.string().min(1).optional(), leagueId: z.string().uuid().optional(), wagerId: z.string().uuid(), reviewedByDiscordId: z.string().min(1) }).parse(request.body);
      ...
      return reply.send(await settleWager({ ...body, leagueId: leagueId ?? undefined }));
    } catch (error) {
      return sendError(reply, error);
    }
  });
  ...
  app.post("/v1/wagers/reopen-game", async (request, reply) => {
    try {
      const body = z.object({ guildId: z.string().min(1), gameId: z.string().uuid() }).parse(request.body);
      await requireBotOrUserSession(request, { resolveGuildId: () => body.guildId, permission: "co_commissioner" });
      return reply.send(await reopenWageringForGame(body));
    } catch (error) { return sendError(reply, error); }
  });
