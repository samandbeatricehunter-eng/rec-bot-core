...
export async function auditAndRepairRecords(guildId: string) {
  ...
  for (const g of completed(seasonGames ?? [])) {
    if (g.home_user_id && g.away_user_id) { globalUserIds.add(g.home_user_id); globalUserIds.add(g.away_user_id); }
  }
  ...
  if (globalUserIds.size > 0) {
    ...
    for (const g of completed(allH2HGames ?? [])) {
      ...
      const ids = [g.home_user_id, g.away_user_id].sort() as [string, string];
      const key = h2hKey(ids[0], ids[1]);
      if (g.home_user_id) { if (!globalTotals.has(g.home_user_id)) globalTotals.set(g.home_user_id, freshStats()); accumulate(globalTotals.get(g.home_user_id)!, home, away); }
      if (g.away_user_id) { if (!globalTotals.has(g.away_user_id)) globalTotals.set(g.away_user_id, freshStats()); accumulate(globalTotals.get(g.away_user_id)!, away, home); }
