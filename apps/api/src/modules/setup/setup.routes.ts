...
const CreateUnclaimedLeagueSchema = z.object({
  ...
  templateId: z.enum(["rec_recommended", "normal_regs", "hardcore_regs", "normal_fantasy", "fantasy_free_for_all"]).optional().nullable(),
  initialTeamAbbreviation: z.string().trim().min(1).max(12),
  ...
  currentWeek: z.number().int().min(0).max(30).optional(),
