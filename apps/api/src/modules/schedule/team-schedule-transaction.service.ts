...
export async function commitTeamScheduleDecisionsTransactional(input: {
  ...
  try {
    ...
    for (const game of relevantExisting) {
      ...
      const locked = Boolean(game.has_result);
      if (locked && !exact) {
        throw new ApiError(409, `Week ${game.week_number} already has a recorded result; its participants cannot be changed.`);
      }
...
export async function removeTeamScheduleGame(input: {
  ...
  try {
    ...
    if (game.has_result) {
      throw new ApiError(409, "This game already has a recorded result — reverse that first before removing it.");
    }
