...
async function userIdForDiscord(discordId: string) {
  ...
  // a synthetic discordId) use the site-only-id convention.
  if (isSiteOnlyDiscordId(discordId)) return recUserIdFromSiteOnlyDiscordId(discordId);
  return withComputeCache(`discord-user:${discordId}`, 60_000, async () => {
...
export async function getHub(guildId: string, discordId: string) {
  // Daily prune can take tens of seconds on a cold process (Cloudflare HEAD checks). Never
  // block the hub payload on it — a first-of-day / post-deploy hub open used to wait on the
  // whole sweep.
  void pruneDeadHighlightsOnceDaily().catch((error) => {
...
async function loadHub(guildId: string, discordId: string) {
  ...
  const [announcements, headlines, highlights, matchups, myTeam, powerRankings, sos, userRatings, storeConfig, economy, weeklyBundle, currentStreamLogs, seasonId, membership] = await Promise.all([
    ...
    userId ? Promise.all([
      ...
      supabase.from("rec_dollar_ledger").select("amount,transaction_type,source_reference").eq("league_id", context.leagueId).eq("user_id", userId).in("transaction_type", ["box_score_payout", "game_result_payout", "immortality_media_day_payout"]),
    ]) : Promise.resolve([emptyWeekly, emptyWeekly, emptyWeekly, emptyWeekly] as const),
    supabase
  ...
  const commissionerTier: "commissioner" | "co_commissioner" | null = !canManageLeague
    ...
    : ["co_commissioner", "co"].includes(membershipRole) && !isOwner
      ? "co_commissioner"
      : "commissioner";
  const [weeklyHighlights, weeklyStreams, weeklyGotwVotes, weeklyLedgers] = weeklyBundle;
  ...
  const [reactions, views, storyReactions, storyComments, gameReactions, highlightGameUsers, liveGameTeamsRes, streamViews, streamReactions] = await Promise.all([
    ...
    liveGameTeamIds.length
      ? supabase.from("rec_teams").select("id,name,abbreviation,display_city,display_nick,is_relocated").in("id", liveGameTeamIds)
      : Promise.resolve({ data: [] as any[], error: null }),
  ...
  let weeklyItems = [
    ...
    // Non-RTI's own Media Day interview (a team's coach answering that week's questions) was
    // removed -- it had no UI path left to reach it after the Media Day rebuild superseded it
    // (see media-day/media-day-interview.service.ts), so nothing could ever earn this payout.
    ...(isRise ? [
...
export async function getHubMatchupSchedule(input: { guildId: string; discordId: string; weekNumber?: number | null; seasonNumber?: number | null }) {
  ...
  if (String(context.rec_leagues.game ?? "").startsWith("madden_")) {
    ...
    const colorMap = NFL_TEAM_PRIMARY_COLORS;
  ...
  const gameReactionsForWeek = gameIds.length
    ? await supabase.from("rec_game_reactions").select("game_id,user_id,reaction_key,comment").in("game_id", gameIds)
    : { data: [], error: null };
