...
export const recApi = {
  ...
  importMaddenEaDatasets: (input: { guildId: string; leagueId: string; connectionId: string; datasets?: EaDataset[]; weekRefs?: Array<{ stage: 0 | 1; weekIndex: number }>; weekScope?: "current" | "through_current" | "full_season" }) =>
  ...
  getNflPlayoffPicture: (guildId: string) =>
    recApiFetch<import("../types/api.js").NflPlayoffPicture>("/v1/standings/nfl-playoff-picture", { method: "POST", body: JSON.stringify({ guildId }), cacheTtlMs: 120_000 }),
  getNflPlayoffBracketSnapshot: (guildId: string) =>
    recApiFetch<{ seasonNumber: number; picture: import("../types/api.js").NflPlayoffPicture } | null>("/v1/standings/nfl-playoff-bracket-snapshot", { method: "POST", body: JSON.stringify({ guildId }), cacheTtlMs: 120_000 }),
  ...
  resolveImmortalityTreePurchase: (input: { guildId: string; requestId: string; action: "applied" | "refunded"; note?: string }) =>
    recApiFetch<any>("/v1/immortality/progression/resolve", { method: "POST", body: JSON.stringify(input) }),
  resolveImmortalityDevPromotion: (input: { guildId: string; requestId: string; action: "applied" | "refunded"; note?: string }) =>
    recApiFetch<any>("/v1/immortality/progression/dev-promotion/resolve", { method: "POST", body: JSON.stringify(input) }),
  resolveImmortalityOwnerTreePurchase: (input: { guildId: string; requestId: string; action: "applied" | "refunded"; note?: string }) =>
    recApiFetch<any>("/v1/immortality/owner-progression/resolve", { method: "POST", body: JSON.stringify(input) }),
  resolveImmortalityAbilityChange: (input: { guildId: string; requestId: string; action: "applied" | "refunded"; note?: string }) =>
    recApiFetch<any>("/v1/immortality/abilities/resolve", { method: "POST", body: JSON.stringify(input) }),
